...
def main():
    ...
    checkpoint_path = Path(data_config.checkpoints_path)
    ...
    train_sampler = grain.samplers.IndexSampler(
        num_records=len(processed_train_dataset),
        shuffle=False,
        shard_options=grain.sharding.ShardOptions(shard_index=0, shard_count=1, drop_remainder=True)
    )
    
    train_loader = grain.DataLoader(
        data_source=processed_train_dataset,
        sampler=train_sampler,
        worker_count=data_config.worker_count,
    ...
    # Pre-fetch one batch to warm up JIT? Or just use the loop.
    # The original code did a manual fetch.
    # We can skip manual fetch or do:
    ...
    try:
        ...
        # Could be end of data if dataset is empty, but unlikely.
    ...
    for epoch in range(train_config.num_epochs):
        ...
        for element in pbar:
            ...
            # if train_config.val_every_n_steps and global_step % train_config.val_every_n_steps == 0:
            #     val_loss = run_validation(model, test_loader)
