...
def process_tsv(
    tsv_filename: Path, root_path: Path, resampled_path: Path, max_workers: int
):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as workers:
        futures = {
            workers.submit(resample_and_write, row, resampled_path)
    ...
    processed_audios["label"] = (
        processed_audios["label"]
        .str.translate(remove_punct_map)
        .str.strip()
        .str.lower()
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="root path for common voice dataset",
    )
    parser.add_argument(
        "--tsv_filenames",
        type=Path,
        nargs="+",
        required=True,
        help="tsv files from which we generate data",
    )
    parser.add_argument(
        "--max_workers", type=int, required=False, default=min(14, os.cpu_count() or 1)
    )
